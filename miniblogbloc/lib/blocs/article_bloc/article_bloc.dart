...
class ArticleBloc extends Bloc<ArticleEvent, ArticleState> {
  ...
  void _onAdd(AddArticle event, Emitter<ArticleState> emit) async {
    bool success = await articleRepository.addBlog(
        event.selectedImage, event.title, event.content, event.author);

    if (success) {
      emit(ArticleAdded());
      emit(
          ArticlesInitial()); //veri ekledikten sonra bilinmedik durum çıkıyordu onu engellemek için
    } else {
      emit(ArticleAddedError());
    }
  }

  // void _onReset(ResetFetchArticleAdd event, Emitter<ArticleState> emit) async {
